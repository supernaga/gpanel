<template>
  <section>
    ...
    <table>
      <thead><tr><th>ID</th><th>节点UID</th><th>节点名</th><th>命令</th><th>状态</th><th>重试</th><th>超时</th><th>结果</th><th>创建时间</th></tr></thead>
      <tbody>
        <tr v-for="t in tasks" :key="t.id">
          <td>{{ t.id }}</td><td>{{ t.nodeUid }}</td><td>{{ t.nodeName }}</td><td>{{ t.command }}</td><td>{{ t.status }}</td><td>{{ t.retryCount }}/{{ t.maxRetries }}</td><td>{{ t.timeoutSecs }}s</td><td>{{ t.result }}</td><td>{{ new Date(t.createdAt).toLocaleString() }}</td>
        </tr>
      </tbody>
    </table>
  </section>
...
const tasks = ref([])
const form = ref({ nodeUid: '', nodeName: '', command: '', payload: '{}', maxRetries: 3, timeoutSecs: 300 })
const load = async () => tasks.value = await api.agentTasks()
const create = async () => { await api.addAgentTask(form.value); form.value = { nodeUid: '', nodeName: '', command: '', payload: '{}', maxRetries: 3, timeoutSecs: 300 }; await load() }
onMounted(load)
</script>
