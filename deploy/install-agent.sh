...
if [ -z "$PANEL" ] || [ -z "$TOKEN" ]; then
  echo "Usage: bash install-agent.sh --panel http://PANEL_IP --token AGENT_TOKEN [--name node-01] [--image ghcr.io/.../gpanel-agent:latest]"
  exit 1
fi
...
docker run -d --name gpanel-agent --restart always \
  ...
  -e AGENT_VERSION="v0.2.0" \
  --network host \
  "$IMAGE"

echo "[OK] agent started with image: $IMAGE"
